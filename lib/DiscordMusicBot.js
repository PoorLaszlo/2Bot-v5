    ...
    this.manager = new Manager({
      ...
      .on("trackError", (player, err) => {
        ...
        let errorEmbed = new MessageEmbed()
          .setColor("RED")
          .setTitle("Visszajátszás hiba!")
          .setDescription(`Sikertelen a zene betöltése: \`${song.title}\``)
          .setFooter({
            text: "Hoppá! Valami közbelépett, de nem a te hibád!",
          });
            ...
            channel.send({
              embeds: [
                new MessageEmbed()
                  ...
                  .setDescription(`Kilépve innen: <#${oldChannel}>`),
              ],
            });
      ...
      .on("trackStart", async (player, track) => {
        ...
        let trackStartedEmbed = this.Embed()
          .setAuthor({ name: "Éppen megy", iconURL: this.config.iconURL })
          .setDescription(`[${track.title}](${track.uri})` || "Nincs leírás")
          .addField("Kérte", `${track.requester}`, true)
      ...
      .on("queueEnd", (player) => {
        ...
        let queueEmbed = this.Embed()
          .setAuthor({
            name: "A várólista véget ért",
            iconURL: this.config.iconURL,
          })
          .setFooter({ text: "Várólista véget ért:" })
          .setTimestamp();
        client.channels.cache
          .get(player.textChannel)
            ...
            setTimeout(() => {
              if (!player.playing && player.state !== "DISCONNECTED") {
                let disconnectedEmbed = new MessageEmbed()
                  ...
                  .setDescription(
                    `A bot inaktívitás miatt kilépett.`
                  );
                client.channels.cache
                  .get(player.textChannel)
